    ...
    def delete(self, data):
        ...
        if current.get_left() is None and current.get_right() is None:
            ...
            if before is None:
                ...
                before.set_left(current.get_left())
            else:
                before.set_right(current.get_left())
        elif current.get_left() is None and current.get_right() is not None:
            #only right#
            if before is None:
                self.root = current.get_right()
            elif before.get_left() == current:
                before.set_left(current.get_right())
            else:
                before.set_right(current.get_right())
        else:
            #left and right#
            target = current
            swapper = current.get_left()

            while swapper.get_right(): # max of left
                target = swapper
                swapper = swapper.get_right()

            current.set_data(swapper.get_data())

            if target.get_left() == swapper: # move up if swapper have child
                target.set_left(swapper.get_left())
            else:
                target.set_right(swapper.get_left())
